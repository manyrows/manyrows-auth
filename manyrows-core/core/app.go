...
// AppDisplayName composes the user-visible app label from a project
...
func AppDisplayName(projectName, envType string) string {
	if projectName == "" {
		return "(unnamed app)"
	}
	switch envType {
...
func (a *App) DisplayName() string {
	return AppDisplayName(a.ProjectName, a.Type)
}
...
type App struct {
	...
	// ProjectName is the parent project's name, populated by repo
	// JOIN/subquery on read. Not persisted on the apps table - the
	// display name is computed from project + env type.
