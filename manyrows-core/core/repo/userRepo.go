...
func (r *Repo) ListUsersInProduct(ctx context.Context, productID, workspaceID uuid.UUID, emailQuery string, limit, offset int) ([]core.User, error) {
	_ = workspaceID
...
// CountUsersInProduct counts distinct users who are members of any app
// in the product, honoring the same optional email filter as
// ListUsersInProduct.
func (r *Repo) CountUsersInProduct(ctx context.Context, productID, workspaceID uuid.UUID, emailQuery string) (int, error) {
	_ = workspaceID
...
// AppName is composed in Go from product + env type at scan time so
...
func (r *Repo) GetUserAppAccessForWorkspace(ctx context.Context, workspaceID uuid.UUID) (map[uuid.UUID][]UserAppAccess, error) {
	const q = `
SELECT au.user_id, a.id, p.name, a.type, a.product_id
FROM app_users au
JOIN apps a ON a.id = au.app_id
...
`
	...
	for rows.Next() {
		...
		var productName, envType string
		if err := rows.Scan(&ua.UserID, &ua.AppID, &productName, &envType, &ua.ProductID); err != nil {
			return nil, err
		}
		...
		ua.AppName = core.AppDisplayName(productName, envType)
