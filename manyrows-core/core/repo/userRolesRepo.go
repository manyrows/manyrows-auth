...
// GetUserRolesByProjectID returns all role assignments for a project.
// user_roles no longer carries project_id directly; the project filter
// is applied via apps.
func (r *Repo) GetUserRolesByProjectID(ctx context.Context, projectID uuid.UUID) ([]core.UserRole, error) {
	const q = `
		select
			ur.id,
			ur.app_id,
...
func (r *Repo) GetUserRolesByUserID(ctx context.Context, projectID, userID uuid.UUID) ([]core.UserRole, error) {
	const q = `
		select
			ur.id,
			ur.app_id,
	...
	rows, err := r.db.Pool().Query(ctx, q, projectID, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) GetRoleSlugsAndPermissionSlugsForRoleIDs(
	...
	roleRows, err := r.db.Pool().Query(ctx, qRoles, projectID, roleIDs)
	if err != nil {
		return nil, nil, err
	}
	...
	permRows, err := r.db.Pool().Query(ctx, qPerms, projectID, roleIDs)
	if err != nil {
		return nil, nil, err
	}
...
// projectID parameter is kept for API stability but no longer needed for
// the query — app_id implies the project (apps.project_id).
...
type ReplaceUserRolesParams struct {
	ProjectID uuid.UUID // used to validate roles belong to this project
	UserID    uuid.UUID
...
// Validates that AppID belongs to ProjectID and that all RoleIDs belong
// to ProjectID — both checks guard against an admin in project A
...
func (r *Repo) ReplaceUserRoles(ctx context.Context, p ReplaceUserRolesParams) error {
	...
	// Validate app belongs to project.
	{
		...
		`
		var one int
		if err := tx.QueryRow(ctx, q, p.AppID, p.ProjectID).Scan(&one); err != nil {
	...
	if len(p.RoleIDs) > 0 {
		...
		`
		var cnt int
		if err := tx.QueryRow(ctx, q, p.ProjectID, roleIDs).Scan(&cnt); err != nil {
			return err
		}
		if cnt != len(roleIDs) {
			return ErrBadRequest
...
// ensuring roleID belongs to the app's project (resolveRoleSlugs does this).
...
func (r *Repo) ProjectExists(ctx context.Context, projectID uuid.UUID) (bool, error) {
	const q = `select 1 from projects where id = $1`
	var one int
	err := r.db.Pool().QueryRow(ctx, q, projectID).Scan(&one)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return false, nil
		}
...
func (r *Repo) GetValidAppIDs(ctx context.Context, projectID uuid.UUID, appIDs []uuid.UUID) ([]uuid.UUID, error) {
	if len(appIDs) == 0 {
	...
	const q = `
		...
		where project_id = $1
		  and id = any($2::uuid[])
	`
	rows, err := r.db.Pool().Query(ctx, q, projectID, appIDs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) GetValidRoleIDs(ctx context.Context, projectID uuid.UUID, roleIDs []uuid.UUID) ([]uuid.UUID, error) {
	if len(roleIDs) == 0 {
		return []uuid.UUID{}, nil
	}
	const q = `
		...
		where project_id = $1
		  and id = any($2::uuid[])
	`
	rows, err := r.db.Pool().Query(ctx, q, projectID, roleIDs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *Repo) GetRoleNames(ctx context.Context, projectID uuid.UUID, roleIDs []uuid.UUID) ([]string, error) {
	if len(roleIDs) == 0 {
		return []string{}, nil
	}
	const q = `
...
// user for an app (not via roles). projectID kept for API stability — the
// query no longer needs it since app_id implies the project.
func (r *Repo) GetDirectPermissionSlugs(ctx context.Context, projectID, userID, appID uuid.UUID) ([]string, error) {
	_ = projectID
	const q = `
		SELECT DISTINCT p.slug
...
func (r *Repo) GetDirectPermissionIDs(ctx context.Context, projectID, userID, appID uuid.UUID) ([]uuid.UUID, error) {
	_ = projectID
	const q = `
...
func (r *Repo) SetDirectPermissions(ctx context.Context, projectID, userID, appID uuid.UUID, permissionIDs []uuid.UUID) error {
	if appID == uuid.Nil {
		return ErrBadRequest
	}
	tx, err := r.db.Pool().Begin(ctx)
	...
	{
		var one int
		if err := tx.QueryRow(ctx, `select 1 from apps where id = $1 and project_id = $2`, appID, projectID).Scan(&one); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return ErrBadRequest
			}
			return err
	...
	if len(permissionIDs) > 0 {
		...
		var cnt int
		if err := tx.QueryRow(ctx, `select count(*) from permissions where project_id = $1 and id = any($2::uuid[])`, projectID, permIDs).Scan(&cnt); err != nil {
			return err
		}
		if cnt != len(permIDs) {
			return ErrBadRequest
