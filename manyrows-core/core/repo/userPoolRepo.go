...
// project context and member counts. The admin drill-down dialog
...
`
	...
	for rows.Next() {
		var pa PoolApp
		if err := rows.Scan(&pa.ID, &pa.ProjectID, &pa.ProjectName, &pa.Type, &pa.Enabled, &pa.MemberCount); err != nil {
			return nil, err
		}
		pa.DisplayName = core.AppDisplayName(pa.ProjectName, pa.Type)
		out = append(out, pa)
	}
	return out, rows.Err()
}
