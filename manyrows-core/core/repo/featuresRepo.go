...
func (r *Repo) GetFeatureFlagsByProductID(ctx context.Context, productID uuid.UUID) ([]core.FeatureFlag, error) {
	const q = `
		select
			id,
			product_id,
...
func (r *Repo) GetFeatureFlagByID(ctx context.Context, productID uuid.UUID, featureFlagID uuid.UUID) (core.FeatureFlag, error) {
	const q = `
		select
			id,
			product_id,
	...
	var desc *string
	var scope string
	err := r.db.Pool().QueryRow(ctx, q, productID, featureFlagID).Scan(
		&ff.ID,
		&ff.ProductID,
		&ff.Key,
		&desc,
...
func (r *Repo) GetFeatureFlagByProductIDAndKey(ctx context.Context, productID uuid.UUID, key string) (core.FeatureFlag, error) {
	const q = `
		select
			id,
			product_id,
			...
			created_by_account_id
		from feature_flags
		where product_id = $1 and key = $2 and status = 'active'
	`

	var ff core.FeatureFlag
	var desc *string
	var scope string
	err := r.db.Pool().QueryRow(ctx, q, productID, key).Scan(
		&ff.ID,
		&ff.ProductID,
		&ff.Key,
		&desc,
...
func (r *Repo) UpdateFeatureFlag(
	...
	if len(sets) == 0 {
		return r.GetFeatureFlagByID(ctx, productID, featureFlagID)
	}
...
func (r *Repo) DeleteFeatureFlag(ctx context.Context, productID uuid.UUID, featureFlagID uuid.UUID) error {
	const q = `
...
func (r *Repo) GetFeatureFlagOverridesByProductID(ctx context.Context, productID uuid.UUID) ([]core.FeatureFlagOverride, error) {
	const q = `
		select
			id,
			product_id,
...
func (r *Repo) GetFeatureFlagOverride(
	...
	var out core.FeatureFlagOverride
	err := r.db.Pool().QueryRow(ctx, q, productID, featureFlagID, appID).Scan(
		&out.ID,
		&out.ProductID,
		&out.AppID,
		&out.FeatureFlagID,
...
func (r *Repo) UpsertFeatureFlagOverride(ctx context.Context, o core.FeatureFlagOverride) (core.FeatureFlagOverride, error) {
	...
	{
		const q = `select 1 from feature_flags where id = $1 and product_id = $2`
		var one int
		err := r.db.Pool().QueryRow(ctx, q, o.FeatureFlagID, o.ProductID).Scan(&one)
		if err != nil {
			if isNoRows(err) {
	...
	{
		const q = `select 1 from apps where id = $1 and product_id = $2`
		var one int
		err := r.db.Pool().QueryRow(ctx, q, o.AppID, o.ProductID).Scan(&one)
		if err != nil {
			if isNoRows(err) {
				return core.FeatureFlagOverride{}, ErrNotFound
			}
...
func (r *Repo) DeleteFeatureFlagOverride(ctx context.Context, productID uuid.UUID, featureFlagID uuid.UUID, appID uuid.UUID) error {
	const q = `
...
// GetEvaluatedFeatureFlagsForProductAndApp returns feature flags for a project
...
func (r *Repo) GetEvaluatedFeatureFlagsForProductAndApp(
	ctx context.Context,
	productID uuid.UUID,
	appID uuid.UUID,
...
left join feature_flag_overrides ffe
  on ffe.product_id = ff.product_id
 and ffe.feature_flag_id = ff.id
 and ffe.app_id = $2
where ff.product_id = $1
order by ff.key asc;
`

	rows, err := r.db.Pool().Query(ctx, q, productID, appID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
