...
`
	...
	for rows.Next() {
		...
		sr.User = &usr
		if appID != nil && *appID != uuid.Nil && productName != nil && appType != nil {
			sr.App = &core.ClientSessionApp{ID: *appID, Name: core.AppDisplayName(*productName, *appType)}
		}
		out = append(out, sr)
	}
