...
func (r *Repo) GetConfigKeysByProductID(ctx context.Context, productID uuid.UUID) ([]core.ConfigKey, error) {
	const q = `
		select
			id,
			product_id,
...
func (r *Repo) GetConfigKeyByID(ctx context.Context, productID, configKeyID uuid.UUID) (core.ConfigKey, error) {
	const q = `
		select
			id,
			product_id,
	...
	err := r.db.Pool().QueryRow(ctx, q, productID, configKeyID).Scan(
		&out.ID,
		&out.ProductID,
		&out.Key,
		&desc,
...
func (r *Repo) GetConfigKeyByProductIDAndKey(ctx context.Context, productID uuid.UUID, key string) (core.ConfigKey, error) {
	const q = `
		select
			id,
			product_id,
...
func (r *Repo) UpdateConfigKey(
	...
	args = append(args, productID, configKeyID)

	q := `
		update config_keys
		set ` + strings.Join(set, ", ") + `
		where product_id = $` + itoa(argn) + ` and id = $` + itoa(argn+1) + `
		returning
			id,
			product_id,
			key,
...
func (r *Repo) GetConfigValuesByProductID(ctx context.Context, productID uuid.UUID) ([]core.ConfigValue, error) {
	const q = `
		select
			id,
			product_id,
...
func (r *Repo) UpsertConfigValueJSON(
	...
	const metaQ = `
		select
			...
			ck.value_type
		from config_keys ck
		join apps a on a.id = $2 and a.product_id = $1
		where ck.id = $3 and ck.product_id = $1
	`
	var exposure string
	var vt string
	if err := r.db.Pool().QueryRow(ctx, metaQ, cv.ProductID, cv.AppID, cv.ConfigKeyID).Scan(&exposure, &vt); err != nil {
		if isNoRows(err) {
...
func (r *Repo) DeleteConfigValue(ctx context.Context, productID, configKeyID, appID uuid.UUID) error {
	const q = `
...
func (r *Repo) GetPublicConfigForProductAndApp(
	ctx context.Context,
	productID uuid.UUID,
	appID uuid.UUID,
	...
	rows, err := r.db.Pool().Query(ctx, q, productID, appID, core.ConfigExposurePublic)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
