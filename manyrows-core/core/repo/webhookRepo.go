...
func (r *Repo) GetWebhooksByAppID(ctx context.Context, appID uuid.UUID) ([]core.Webhook, error) {
	const q = `
		SELECT id, product_id, app_id, url, secret, events, status, description, created_at, updated_at, created_by
		FROM webhooks
		WHERE app_id = $1
...
func (r *Repo) InsertWebhook(ctx context.Context, w core.Webhook) error {
	const q = `
		INSERT INTO webhooks (
			id, product_id, app_id, url, secret, events, status, description, created_at, updated_at, created_by
		) VALUES (
