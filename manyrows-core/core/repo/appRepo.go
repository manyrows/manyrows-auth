...
func (r *Repo) InsertApp(ctx context.Context, a core.App) (core.App, error) {
	...
	if a.WorkspaceID == uuid.Nil || a.ProductID == uuid.Nil {
...
from apps
	...
	rows, err := r.db.Pool().Query(ctx, q, workspaceID, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// the computed product + env-type label.
func (r *Repo) UpdateAppEnabled(ctx context.Context, workspaceID, productID, appID uuid.UUID, enabled bool, u AppCoreUpdate) (core.App, error) {
	const q = `
		update apps
		set enabled = $4,
		    app_url = $5,
...
func (r *Repo) UpdateAppRegistration(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppRegistrationUpdate) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppPrimaryAuthMethod(ctx context.Context, workspaceID, productID, appID uuid.UUID, primaryAuthMethod string) (core.App, error) {
	q := `
		update apps
...
// caller's job.
func (r *Repo) UpdateAppCookieDomain(ctx context.Context, workspaceID, productID, appID uuid.UUID, cookieDomain *string) (core.App, error) {
	q := `
		update apps
		set cookie_domain = $4,
		    updated_at = now()
...
func (r *Repo) UpdateAppTransportMode(ctx context.Context, workspaceID, productID, appID uuid.UUID, mode string) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppPasswordPolicy(ctx context.Context, workspaceID, productID, appID uuid.UUID, minLength, minScore int) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppGoogleConfig(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppGoogleConfigUpdate) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppAppleConfig(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppAppleConfigUpdate) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppMicrosoftConfig(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppMicrosoftConfigUpdate) (core.App, error) {
	q := `
		update apps
...
// atomically.
func (r *Repo) UpdateAppGithubConfig(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppGithubConfigUpdate) (core.App, error) {
	q := `
		update apps
...
func (r *Repo) UpdateAppNaverConfig(ctx context.Context, workspaceID, productID, appID uuid.UUID, u AppNaverConfigUpdate) (core.App, error) {
	q := `
		update apps
	...
	err := scanAppFull(r.db.Pool().QueryRow(ctx, q,
		appID, workspaceID, productID,
		u.AuthMethodNaver, u.ClientID, u.ClientSecretEncrypted,
	), &out)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return core.App{}, ErrNotFound
...
func (r *Repo) DeleteAppByID(ctx context.Context, workspaceID, productID, appID uuid.UUID) error {
	const q = `
		delete from apps
		where id = $1 and workspace_id = $2 and product_id = $3
	`
