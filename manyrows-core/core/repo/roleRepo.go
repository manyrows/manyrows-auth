...
// GetRolesByProjectID returns all roles for a project, each with Permissions loaded.
func (r *Repo) GetRolesByProjectID(ctx context.Context, projectID uuid.UUID) ([]core.Role, error) {
	// 1) Load roles
	const qRoles = `
		select
			id,
	...
	rows, err := r.db.Pool().Query(ctx, qRoles, projectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	// (we ensure project scoping via permissions.project_id = $1)
	const qPerms = `
		select
			rp.role_id,
	...
	rows2, err := r.db.Pool().Query(ctx, qPerms, projectID, roleIDs)
	if err != nil {
		return nil, err
	}
	defer rows2.Close()
...
func (r *Repo) GetRoleByID(ctx context.Context, projectID, roleID uuid.UUID) (core.Role, error) {
	const q = `
		select
			id,
			project_id,
	...
	var role core.Role
	err := r.db.Pool().QueryRow(ctx, q, projectID, roleID).Scan(
		&role.ID,
		&role.ProjectID,
		&role.Name,
		&role.Slug,
	...
	rows, err := r.db.Pool().Query(ctx, qPerms, projectID, roleID)
	if err != nil {
		return core.Role{}, err
	}
	defer rows.Close()
...
func (r *Repo) CreateRole(ctx context.Context, p CreateRoleParams) (core.Role, error) {
	...
	const q = `
		insert into roles (project_id, name, slug, created_at, updated_at, id)
		values ($1, $2, $3, $4, $4, $5)
		returning
			id,
			project_id,
	...
	var role core.Role
	err := r.db.Pool().QueryRow(ctx, q, p.ProjectID, name, slug, p.Now, utils.NewUUID()).Scan(
		&role.ID,
		&role.ProjectID,
		&role.Name,
		&role.Slug,
...
func (r *Repo) UpdateRole(ctx context.Context, p UpdateRoleParams) (core.Role, error) {
	...
	if !setName && !setSlug {
		...
		return r.GetRoleByID(ctx, p.ProjectID, p.RoleID)
	}
	...
	var role core.Role
	err := r.db.Pool().QueryRow(ctx, q, p.ProjectID, p.RoleID, nameVal, slugVal, p.Now).Scan(
		&role.ID,
		&role.ProjectID,
		&role.Name,
		&role.Slug,
...
func (r *Repo) ReplaceRolePermissions(ctx context.Context, p ReplaceRolePermissionsParams) error {
	...
	{
		const q = `select 1 from roles where project_id = $1 and id = $2`
		var one int
		if err := tx.QueryRow(ctx, q, p.ProjectID, p.RoleID).Scan(&one); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return ErrNotFound
			}
			return err
	...
	if len(p.PermissionIDs) > 0 {
		...
		`
		var cnt int
		if err := tx.QueryRow(ctx, q, p.ProjectID, uniqueUUIDs(p.PermissionIDs)).Scan(&cnt); err != nil {
			return err
		}
...
func (r *Repo) DeleteRole(ctx context.Context, projectID, roleID uuid.UUID) error {
	if projectID == uuid.Nil || roleID == uuid.Nil {
		return ErrBadRequest
	}
	...
	// (Optional) ensure exists first so we can return ErrNotFound cleanly.
	{
		const q = `select 1 from roles where project_id = $1 and id = $2`
		var one int
		if err := tx.QueryRow(ctx, q, projectID, roleID).Scan(&one); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				return ErrNotFound
			}
			return err
	...
	{
		const q = `delete from roles where project_id = $1 and id = $2`
		ct, err := tx.Exec(ctx, q, projectID, roleID)
		if err != nil {
...
// CountRolesByProjectID returns the number of roles in a project.
func (r *Repo) CountRolesByProjectID(ctx context.Context, projectID uuid.UUID) (int, error) {
	const q = `SELECT COUNT(*) FROM roles WHERE project_id = $1`
