...
// GetProductMembers returns users who are members of at least one app
...
func (r *Repo) GetProductMembers(
	...
	`
	var total int
	if err := r.db.Pool().QueryRow(ctx, qCount, productID, emailPattern).Scan(&total); err != nil {
		return nil, 0, err
	}
	...
	rows, err := r.db.Pool().Query(ctx, q, productID, emailPattern, pageSize, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
...
func (r *Repo) GetProductMembersByApp(
	...
	if hasApp {
		...
		limitParam = "$4"
		offsetParam = "$5"
		countArgs = []any{productID, appID, emailPattern}
		queryArgs = []any{productID, appID, emailPattern, pageSize, offset}
	} else {
		appFilter = ""
		emailParam = "$2"
		limitParam = "$3"
		offsetParam = "$4"
		countArgs = []any{productID, emailPattern}
		queryArgs = []any{productID, emailPattern, pageSize, offset}
	}
	...
	case "without":
		if hasApp {
			...
			clause := "and not exists (select 1 from user_roles ur where ur.user_id = u.id and ur.app_id in (select id from apps where product_id = $1))"
			countRoleClause = clause
			queryRoleClause = clause
		}
	case "specific":
		...
		if hasApp {
			...
			countRoleClause = fmt.Sprintf("and exists (select 1 from user_roles ur where ur.user_id = u.id and ur.app_id in (select id from apps where product_id = $1) and ur.role_id = $%d)", len(countArgs))
			queryRoleClause = fmt.Sprintf("and exists (select 1 from user_roles ur where ur.user_id = u.id and ur.app_id in (select id from apps where product_id = $1) and ur.role_id = $%d)", len(queryArgs))
		}
	}
