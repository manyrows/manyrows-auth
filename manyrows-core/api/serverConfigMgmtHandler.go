...
func (handler *RequestHandler) ServerGetConfigValue(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Err(err).Msg("ServerGetConfigValue: GetConfigValuesByProductID failed")
		WriteError(w, r, "error.internalError", http.StatusInternalServerError)
		return
	}
...
// roleSlugsForIDs maps role IDs to their slugs within a product (for echoing a
// stored override's targeted roles as slugs, matching the write API).
func (handler *RequestHandler) roleSlugsForIDs(ctx context.Context, productID uuid.UUID, ids []uuid.UUID) ([]string, error) {
	if len(ids) == 0 {
		return []string{}, nil
	}
	roles, err := handler.repo.GetRolesByProductID(ctx, productID)
	if err != nil {
		return nil, err
	}
