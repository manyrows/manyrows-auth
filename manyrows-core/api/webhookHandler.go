...
// adminProductAndApp resolves workspace, project, and app from URL params.
// It verifies the app belongs to the workspace.
func (handler *RequestHandler) adminProductAndApp(w http.ResponseWriter, r *http.Request) (*core.Account, *core.Workspace, *core.Product, *core.App, bool) {
	...
	if app.WorkspaceID != ws.ID || app.ProductID != project.ID {
		WriteError(w, r, "error.notFound", http.StatusNotFound)
		return nil, nil, nil, nil, false
	}
...
// HandleListWebhooks — GET /products/{productId}/apps/{appId}/webhooks
func (handler *RequestHandler) HandleListWebhooks(w http.ResponseWriter, r *http.Request) {
	_, _, _, app, ok := handler.adminProductAndApp(w, r)
	if !ok {
		return
	}
...
// HandleGetAppWebhookHealth — GET /products/{productId}/apps/{appId}/webhooks/health
//
...
// HandleCreateWebhook — POST /products/{productId}/apps/{appId}/webhooks
func (handler *RequestHandler) HandleCreateWebhook(w http.ResponseWriter, r *http.Request) {
	acc, _, project, app, ok := handler.adminProductAndApp(w, r)
	if !ok {
		return
	}
...
// HandleGetWebhook — GET /products/{productId}/apps/{appId}/webhooks/{webhookId}
...
// HandleUpdateWebhook — PATCH /products/{productId}/apps/{appId}/webhooks/{webhookId}
...
// HandleDeleteWebhook — DELETE /products/{productId}/apps/{appId}/webhooks/{webhookId}
...
// HandleListWebhookDeliveries — GET /products/{productId}/apps/{appId}/webhooks/{webhookId}/deliveries
...
// HandleRetryWebhookDelivery — POST /products/{productId}/apps/{appId}/webhooks/{webhookId}/deliveries/{deliveryId}/retry
