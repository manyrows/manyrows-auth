...
func (handler *RequestHandler) requireActiveClientSessionApp(
	w http.ResponseWriter,
	r *http.Request,
) (*core.ClientSession, *clientSessionIdentity, *core.Workspace, *core.App, *core.Product, bool) {
	ctx := r.Context()
...
func (handler *RequestHandler) resolveRolesAndPermissions(
	ctx context.Context, productID, userID, appID uuid.UUID,
) ([]string, []string, error) {
	memberRoles, err := handler.repo.GetUserRolesByUserAndAppID(
		ctx, productID, userID, appID,
	)
	if err != nil {
		return nil, nil, err
	}
	...
	if len(memberRoles) > 0 {
		...
		roles, perms, err = handler.repo.GetRoleSlugsAndPermissionSlugsForRoleIDs(ctx, productID, roleIDs)
		if err != nil {
			return nil, nil, err
		}
	}
	...
	directPerms, err := handler.repo.GetDirectPermissionSlugs(ctx, productID, userID, appID)
	if err != nil {
		return nil, nil, err
	}
...
func (handler *RequestHandler) GetAppData(w http.ResponseWriter, r *http.Request) {
	...
	flags, err := handler.repo.GetEvaluatedFeatureFlagsForProductAndApp(ctx, project.ID, app.ID)
	if err != nil {
	...
	cfg, err := handler.repo.GetPublicConfigForProductAndApp(ctx, project.ID, app.ID)
	if err != nil {
