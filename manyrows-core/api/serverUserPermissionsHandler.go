...
// resolvePermissionSlugs maps permission slugs to IDs within the project,
// de-duplicating; an unknown slug is a 400. Mirrors resolveRoleSlugs.
func (handler *RequestHandler) resolvePermissionSlugs(w http.ResponseWriter, r *http.Request, projectID uuid.UUID, rawSlugs []string) (permIDs []uuid.UUID, slugs []string, ok bool) {
	permIDs = []uuid.UUID{}
	slugs = []string{}
	if len(rawSlugs) == 0 {
	...
	perms, err := handler.repo.GetPermissionsByProjectID(r.Context(), projectID)
	if err != nil {
		log.Err(err).Msg("resolvePermissionSlugs: GetPermissionsByProjectID failed")
		WriteError(w, r, "error.internalError", http.StatusInternalServerError)
