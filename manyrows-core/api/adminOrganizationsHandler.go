...
type setAppOrgMemberStatusRequest struct {
	Status string `json:"status"`
}

// HandleSetAppOrganizationMemberStatus suspends (status="disabled") or
// reactivates (status="active") an org member from the admin panel. A disabled
// member loses all org access on the next request (read-side chokepoints reject
// non-active members). Last-owner protected (409). Mirrors
// HandleSetAppOrganizationMemberTier.
func (handler *RequestHandler) HandleSetAppOrganizationMemberStatus(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	_, appID, ok := handler.adminAppScope(w, r)
	if !ok {
		return
	...
	var body setAppOrgMemberStatusRequest
	if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
		WriteError(w, r, "error.invalidJson", http.StatusBadRequest)
		return
	}
	...
	if err := handler.repo.SetOrganizationMemberStatusGuarded(ctx, org.ID, userID, status); err != nil {
		...
		case errors.Is(err, repo.ErrLastOwner):
			WriteError(w, r, "error.conflict", http.StatusConflict)
		default:
			log.Err(err).Msg("HandleSetAppOrganizationMemberStatus: update failed")
			WriteError(w, r, "error.internalError", http.StatusInternalServerError)
		}
		return
	}
