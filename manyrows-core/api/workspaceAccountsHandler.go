...
func (handler *RequestHandler) HandleGetWorkspaceAccounts(w http.ResponseWriter, r *http.Request) {
	...
	// Optional filters: projectId or poolId, plus an email substring.
	//
	...
	if poolIDStr := strings.TrimSpace(q.Get("poolId")); poolIDStr != "" {
	...
	} else if projectIDStr := strings.TrimSpace(q.Get("projectId")); projectIDStr != "" {
		projectID, err := uuid.FromString(projectIDStr)
		if err != nil {
			WriteError(w, r, "error.badRequest", http.StatusBadRequest)
			return
		}
		users, err = handler.repo.ListUsersInProject(ctx, projectID, ws.ID, emailQuery, limit, offset)
		if err != nil {
		...
		if paginated {
			total, err = handler.repo.CountUsersInProject(ctx, projectID, ws.ID, emailQuery)
			if err != nil {
	...
	for _, u := range users {
		...
		if access, ok := appAccess[u.ID]; ok {
			...
			for _, a := range access {
				apps = append(apps, appRef{ID: a.AppID.String(), Name: a.AppName, ProjectID: a.ProjectID.String()})
			}
		}
		if apps == nil {
			apps = []appRef{}
...
func (handler *RequestHandler) resolveInviteRoles(ctx context.Context, app *core.App, requestedRoleIDs []uuid.UUID) ([]uuid.UUID, error) {
	if len(requestedRoleIDs) > 0 {
		...
		for _, rid := range requestedRoleIDs {
			...
			if _, err := handler.repo.GetRoleByID(ctx, app.ProjectID, rid); err != nil {
				if errors.Is(err, repo.ErrNotFound) {
