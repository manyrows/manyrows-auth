...
func (handler *RequestHandler) HandleGetMemberRoles(w http.ResponseWriter, r *http.Request) {
	...
	roles, err := handler.repo.GetUserRolesByProjectID(r.Context(), project.ID)
	if err != nil {
...
// HandleRemoveProjectMember fully removes a user from an app: clears
...
// repo.ErrBadRequest when appID doesn't belong to projectID.
func (handler *RequestHandler) removeAppMembership(ctx context.Context, projectID, appID, userID uuid.UUID) error {
	...
	if err := handler.repo.SetDirectPermissions(ctx, projectID, userID, appID, []uuid.UUID{}); err != nil {
		return err
	}
