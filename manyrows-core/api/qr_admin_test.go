...
func setupQRAdminRouter(t *testing.T) *chi.Mux {
	...
	wsRouter.Put("/projects/{projectId}/apps/{appId}/qr-sign-in-config", svc.Handler.HandleUpdateAppQRSignInConfig)
	return r
}

func createQRAppFixture(t *testing.T) (*core.Account, *core.Workspace, *core.Project, uuid.UUID, *core.Session, core.TokenClaims) {
	t.Helper()
...
func putQRConfig(t *testing.T, router *chi.Mux, ws *core.Workspace, project *core.Project, appID uuid.UUID, claims core.TokenClaims, body any) *httptest.ResponseRecorder {
	t.Helper()
	...
	req := httptest.NewRequest(http.MethodPut,
		"/admin/workspace/"+ws.ID.String()+"/projects/"+project.ID.String()+"/apps/"+appID.String()+"/qr-sign-in-config",
...
func TestAdminQRConfig_URLSurfacedOnRegularGet(t *testing.T) {
	...
	ws2.Get("/projects/{projectId}/apps/{appId}", svc.Handler.HandleGetApp)

	getReq := httptest.NewRequest(http.MethodGet,
		"/admin/workspace/"+ws.ID.String()+"/projects/"+project.ID.String()+"/apps/"+appID.String(),
		nil)
