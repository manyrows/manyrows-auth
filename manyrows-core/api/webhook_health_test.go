...
func insertWebhook(t *testing.T, appID uuid.UUID, status string) uuid.UUID {
	...
	// webhooks has both product_id and app_id; need a project for the FK.
	// Cheapest: pick the app's product_id from the apps table.
	var productID uuid.UUID
	if err := pool.QueryRow(context.Background(),
		`SELECT product_id FROM apps WHERE id = $1`, appID).Scan(&productID); err != nil {
...
INSERT INTO webhooks (id, product_id, app_id, url, secret, events, status, description, created_at, updated_at, created_by)
VALUES ($1, $2, $3, 'https://example.test/hook', 'secret', ARRAY['user.login']::text[], $4, '', now(), now(), $5)
`, id, productID, appID, status, createdBy); err != nil {
...
func newWebhookHealthApp(t *testing.T) (accID, wsID, appID uuid.UUID) {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(ctx, "DELETE FROM products WHERE id = $1", ap.ProductID)
