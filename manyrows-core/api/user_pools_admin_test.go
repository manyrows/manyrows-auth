...
func setupUserPoolsRouter(t *testing.T) *chi.Mux {
	...
	// Repoint lives under /products/{productId}/apps/{appId}/userPool
	ws.Post("/products/{productId}/apps/{appId}/userPool", svc.Handler.HandleRepointAppUserPool)
	return r
}
...
func TestHandleRepointAppUserPool_BlocksWhenAppHasMembers(t *testing.T) {
	...
	rr := adminJSON(t, router, http.MethodPost,
		"/admin/workspace/"+ws.ID.String()+"/products/"+app.ProductID.String()+"/apps/"+app.ID.String()+"/userPool",
...
func TestHandleRepointAppUserPool_ForeignAppDoesntLeakMemberCount(t *testing.T) {
	...
	// the foreign app's memberCount. With adminAndProduct +
	// GetAppByIDForProduct the foreign id 404s before we read counts.
	router := setupUserPoolsRouter(t)
	...
	otherProduct := testEnv.CreateTestProduct(t, otherWs, attacker, "AttProj", GenerateUniqueSlug("proj"))
	defer testEnv.CleanupTestData(t, &TestFixtures{Account: attacker, Workspace: otherWs})
	...
	rr := adminJSON(t, router, http.MethodPost,
		"/admin/workspace/"+otherWs.ID.String()+"/products/"+otherProduct.ID.String()+"/apps/"+ownerApp.ID.String()+"/userPool",
...
// poolUserFixture spins up ws+product+pool+app and a user. When
