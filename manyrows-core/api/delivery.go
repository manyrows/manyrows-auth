...
func (handler *RequestHandler) GetDeliveryForServer(w http.ResponseWriter, r *http.Request) {
	...
	// -----------------------------
	keys, err := handler.repo.GetConfigKeysByProjectID(ctx, project.ID)
	if err != nil {
	...
	// -----------------------------
	ffs, err := handler.repo.GetFeatureFlagsByProjectID(ctx, project.ID)
	if err != nil {
	...
	evaluated, err := handler.repo.GetEvaluatedFeatureFlagsForProjectAndApp(ctx, project.ID, app.ID)
	if err != nil {
