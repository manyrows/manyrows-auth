...
type adminAppResponse struct {
	...
	NaverOAuthRedirectURI     string `json:"naverOAuthRedirectUri,omitempty"`
	HasNaverClientSecret      bool   `json:"hasNaverClientSecret"`
...
func (handler *RequestHandler) toAdminAppResponse(a core.App, ws *core.Workspace) adminAppResponse {
	resp := adminAppResponse{
		...
		HasNaverClientSecret:     len(a.NaverClientSecretEncrypted) > 0,
	}
	...
	if baseURL != "" && ws != nil {
		...
		resp.NaverOAuthRedirectURI = baseURL + "/x/" + ws.Slug + "/apps/" + a.ID.String() + "/auth/naver/callback"
...
func (handler *RequestHandler) HandleGetApps(w http.ResponseWriter, r *http.Request) {
	...
	apps, err := handler.repo.GetAppsByWorkspaceAndProjectID(r.Context(), ws.ID, projectID)
	if err != nil {
...
func (handler *RequestHandler) HandleCreateApp(w http.ResponseWriter, r *http.Request) {
	...
	if req.UserPoolID != nil && *req.UserPoolID != uuid.Nil {
		...
		// Auto-pool naming derives from the parent project so the admin
		// sees something recognizable in the pools list. Load the project
		// for its name; the new app inherits the project's identity surface.
		project, err := handler.repo.GetProject(r.Context(), projectID, ws.ID)
		if err != nil || project == nil {
			log.Err(err).Msg("failed to load project for pool auto-naming")
			WriteError(w, r, "error.internalError", http.StatusInternalServerError)
			return
		}
		created, err := handler.repo.CreateUserPoolWithUniqueName(r.Context(), ws.ID, project.Name)
		if err != nil {
	...
	if err != nil {
		// (project_id, type) is unique - one app per env per project.
		...
		if repo.IsUniqueViolation(err) {
			utils.WriteJsonWithStatusCode(w, map[string]any{
				"error": "this project already has a " + req.Type + " environment",
...
type AppResource struct {
	...
	NaverEnabled              bool      `json:"naverEnabled,omitempty"`
...
func (handler *RequestHandler) HandleGetAppForAppKit(w http.ResponseWriter, r *http.Request) {
	...
	// Naver is "enabled" when the toggle is on and both creds set.
	naverEnabled := a.AuthMethodNaver &&
...
func (handler *RequestHandler) HandleGetApp(w http.ResponseWriter, r *http.Request) {
	...
	a, err := handler.repo.GetAppByIDForProject(r.Context(), ws.ID, projectID, appID)
	if err != nil {
...
func (handler *RequestHandler) HandleUpdateApp(w http.ResponseWriter, r *http.Request) {
	...
	cur, err := handler.repo.GetAppByIDForProject(r.Context(), ws.ID, projectID, appID)
	if err != nil {
		log.Err(err).Msg("failed to load app")
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.appNotFound", http.StatusNotFound)
	...
	out, err := handler.repo.UpdateAppEnabled(r.Context(), ws.ID, projectID, appID, newEnabled, repo.AppCoreUpdate{
		AppURL:                newAppURL,
...
func (handler *RequestHandler) HandleDeleteApp(w http.ResponseWriter, r *http.Request) {
	...
	if err := handler.repo.DeleteAppByID(r.Context(), ws.ID, projectID, appID); err != nil {
...
// resolvePathIDs reads projectId + appId from the URL, writing the
...
func (handler *RequestHandler) resolvePathIDs(w http.ResponseWriter, r *http.Request) (projectID, appID uuid.UUID, ok bool) {
	projectID, err := utils.GetPathUUID("projectId", r)
	if err != nil || projectID == uuid.Nil {
		log.Err(err).Msg("failed to parse project id")
		WriteError(w, r, "error.invalidProjectId", http.StatusBadRequest)
...
// Apple, Microsoft, GitHub, Kakao, Naver — with the prospective post-save
// state of all seven.
//
// Rules:
...
func (handler *RequestHandler) requireAtLeastOneSignInMethod(ctx context.Context, ws *core.Workspace, isSuper bool, primaryAuthMethod string, google, apple, microsoft, github, kakao, naver bool) bool {
	...
	case core.PrimaryAuthMethodNone:
		return google || apple || microsoft || github || kakao || naver
	}
	return false
}
...
func (handler *RequestHandler) HandleUpdateAppRegistration(w http.ResponseWriter, r *http.Request) {
	...
	if req.DefaultRoleID != nil && *req.DefaultRoleID != uuid.Nil {
		if _, err := handler.repo.GetRoleByID(r.Context(), projectID, *req.DefaultRoleID); err != nil {
	...
	curApp, curAppErr := handler.repo.GetAppByIDForProject(r.Context(), ws.ID, projectID, appID)
	...
	out, err := handler.repo.UpdateAppRegistration(r.Context(), ws.ID, projectID, appID, repo.AppRegistrationUpdate{
...
func (handler *RequestHandler) HandleUpdateAppAuthMethodConfig(w http.ResponseWriter, r *http.Request) {
	...
	if !handler.requireAtLeastOneSignInMethod(r.Context(), ws, acc.IsSuper(), method, curApp.AuthMethodGoogle, curApp.AuthMethodApple, curApp.AuthMethodMicrosoft, curApp.AuthMethodGithub, curApp.AuthMethodKakao, curApp.AuthMethodNaver) {
	...
	out, err := handler.repo.UpdateAppPrimaryAuthMethod(r.Context(), ws.ID, projectID, appID, method)
	if err != nil {
...
func (handler *RequestHandler) HandleUpdateAppCookieDomain(w http.ResponseWriter, r *http.Request) {
	...
	out, err := handler.repo.UpdateAppCookieDomain(r.Context(), ws.ID, projectID, appID, stored)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.appNotFound", http.StatusNotFound)
			return
...
func (handler *RequestHandler) HandleUpdateAppTransportMode(w http.ResponseWriter, r *http.Request) {
	...
	out, err := handler.repo.UpdateAppTransportMode(r.Context(), ws.ID, projectID, appID, mode)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.appNotFound", http.StatusNotFound)
			return
...
func (handler *RequestHandler) HandleUpdateAppSessionCookieSameSite(w http.ResponseWriter, r *http.Request) {
	...
	out, err := handler.repo.UpdateAppSessionCookieSameSite(r.Context(), ws.ID, projectID, appID, mode)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.appNotFound", http.StatusNotFound)
			return
