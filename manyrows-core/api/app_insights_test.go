...
func newInsightsFixture(t *testing.T) *insightsFixture {
	...
	project := testEnv.CreateTestProduct(t, ws, acc, "Insights Product", GenerateUniqueSlug("proj"))
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(ctx, "DELETE FROM products WHERE id IN ($1, $2)", project.ID, app.ProductID)
...
// TestGetProductMembersActivityCounts exercises the per-user batch
...
func TestGetProductMembersActivityCounts(t *testing.T) {
	fix := newInsightsFixture(t)
	ctx := context.Background()
	pool := testEnv.DB.Pool()
	...
	counts, err := testEnv.Repo.GetProductMembersActivityCounts(ctx, fix.app.ID, []uuid.UUID{userA, userB})
	if err != nil {
		t.Fatalf("GetProductMembersActivityCounts: %v", err)
	}
...
func TestInsightsCrossWorkspace_Returns404(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(ctx, "DELETE FROM products WHERE id = $1", appB.ProductID)
	...
	url := fmt.Sprintf("/admin/workspace/%s/products/%s/apps/%s/insights/summary",
		wsA.ID, appB.ProductID, appB.ID)
	req := httptest.NewRequest(http.MethodGet, url, nil)
