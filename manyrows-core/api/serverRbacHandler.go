...
// Definition CRUD for the project's RBAC, over the S2S API — lets a backend
...
// project metadata: the key's app scopes to the project, so there's no
// per-user membership gate.

func (handler *RequestHandler) serverProjectCtx(w http.ResponseWriter, r *http.Request) (*core.Project, bool) {
...
func (handler *RequestHandler) serverRoleSummary(r *http.Request, projectID uuid.UUID, slug string) (ServerRoleSummary, bool, error) {
	roles, err := handler.repo.GetRolesByProjectID(r.Context(), projectID)
	if err != nil {
...
// ServerCreateRole defines a new role in the project, optionally with permissions.
...
func (handler *RequestHandler) ServerCreateRole(w http.ResponseWriter, r *http.Request) {
	...
	role, err := handler.repo.CreateRole(r.Context(), repo.CreateRoleParams{
		ProjectID: project.ID, Name: req.Name, Slug: req.Slug, Now: time.Now().UTC(),
	})
	if err != nil {
		switch {
	...
	if len(permIDs) > 0 {
		if err := handler.repo.ReplaceRolePermissions(r.Context(), repo.ReplaceRolePermissionsParams{
			ProjectID: project.ID, RoleID: role.ID, PermissionIDs: permIDs, Now: time.Now().UTC(),
		}); err != nil {
...
func (handler *RequestHandler) ServerUpdateRole(w http.ResponseWriter, r *http.Request) {
	...
	if req.Name != nil {
		if _, err := handler.repo.UpdateRole(r.Context(), repo.UpdateRoleParams{
			ProjectID: project.ID, RoleID: roleID, Name: req.Name, Now: time.Now().UTC(),
		}); err != nil {
			switch {
			case errors.Is(err, repo.ErrBadRequest):
				WriteError(w, r, "error.badRequest", http.StatusBadRequest)
	...
	if req.Permissions != nil {
		if err := handler.repo.ReplaceRolePermissions(r.Context(), repo.ReplaceRolePermissionsParams{
			ProjectID: project.ID, RoleID: roleID, PermissionIDs: permIDs, Now: time.Now().UTC(),
		}); err != nil {
...
// ServerCreatePermission defines a new permission in the project.
...
func (handler *RequestHandler) ServerCreatePermission(w http.ResponseWriter, r *http.Request) {
	...
	perm := core.Permission{
		ID: utils.NewUUID(), ProjectID: project.ID, Name: req.Name, Slug: req.Slug, CreatedAt: now, UpdatedAt: now,
	}
...
func (handler *RequestHandler) ServerUpdatePermission(w http.ResponseWriter, r *http.Request) {
	...
	if err := handler.repo.UpdatePermission(r.Context(), core.Permission{
		ID: permIDs[0], ProjectID: project.ID, Name: *req.Name, Slug: slug, UpdatedAt: time.Now().UTC(),
	}); err != nil {
