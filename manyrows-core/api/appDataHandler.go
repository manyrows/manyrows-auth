...
func (handler *RequestHandler) GetAdminAppData(w http.ResponseWriter, r *http.Request) {
	...
	for _, m := range memberships {
		...
		projects, err := handler.repo.GetProjectsByWorkspaceID(ctx, m.ID)
		if err != nil {
			log.Err(err).Msg("failed to get projects by workspace ID for admin app data")
			WriteError(w, r, "error.internalError", http.StatusInternalServerError)
			return
		}
		wrs.Projects = projects
