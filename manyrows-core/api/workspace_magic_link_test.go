...
func configureAppForMagicLink(t *testing.T, app *core.App) *core.App {
	...
	appURL := testAppURL
	updated, err := testEnv.Repo.UpdateAppEnabled(ctx, app.WorkspaceID, app.ProductID, app.ID, app.Enabled, repo.AppCoreUpdate{AppURL: &appURL})
	if err != nil {
		t.Fatalf("set app URL: %v", err)
	}
	updated, err = testEnv.Repo.UpdateAppPrimaryAuthMethod(ctx, app.WorkspaceID, app.ProductID, app.ID, core.PrimaryAuthMethodMagicLink)
	if err != nil {
...
func TestConsumeMagicLink_AutoRegistersWhenAllowed(t *testing.T) {
	...
	defaultRole := createTestRole(t, app.ProductID)
	upd, err := testEnv.Repo.UpdateAppRegistration(ctx, app.WorkspaceID, app.ProductID, app.ID, repo.AppRegistrationUpdate{
