...
import type { AppType, Product, Workspace } from "../core.ts";
...
type AppLite = {
  id: string;
  // productName + type drive the computed display name; the freeform
...
export default function WorkspaceSummary(props: Props) {
  ...
  // project list changes. Cached as a Record<productId, AppLite[]> so
  ...
  // still navigates).
  const [projectApps, setProductApps] = React.useState<Record<string, AppLite[]>>({});
  React.useEffect(() => {
    if (projectCount === 0) {
      setProductApps({});
      return;
        ...
        products.map(async (p) => {
          try {
            const res = await axios.get<{ apps?: AppLite[] } | AppLite[]>(
              `/admin/workspace/${workspace.id}/products/${p.id}/apps/`,
            );
            const data = res.data;
  ...
  }, [workspace.id, products.map((p) => p.id).join(",")]);
  ...
  // state above). Mirrors the lift-from Products.tsx flow that used to
  // live on a dedicated /products page. -----
  ...
  const handleWizardComplete = async (productId: string) => {
    setWizardOpen(false);
    await app.refreshAppData();
    navigate(`/app/workspace/${workspace.id}/products/${productId}`);
  };
  ...
  const doCreate = async () => {
    ...
    if (!name) {
      setCreateErr(t("products.nameRequired"));
      return;
    }
    setCreateBusy(true);
    setCreateErr(null);
    try {
      const res = await axios.post<Product>(
        `/admin/workspace/${workspace.id}/products`,
        { name },
      );
      const created = res.data;
      setCreateOpen(false);
      await app.refreshAppData();
      showToast(t("products.projectCreated"), "success");
      if (created?.id) {
        navigate(`/app/workspace/${workspace.id}/products/${created.id}`);
      }
    } catch (e) {
      setCreateErr(extractApiError(e, t("products.failedToCreate")));
      showToast(t("products.failedToCreate"), "error");
    } finally {
      setCreateBusy(false);
    }
  };
  ...
  return (
    <Stack spacing={3.5}>
      ...
      {/* Products section - list + create. Replaces the standalone
          /products page; the side menu no longer surfaces it
          separately. */}
      {projectCount > 0 && (
        <Box>
          <Divider sx={{ mb: 2.5 }} />
              ...
              return (
                ...
                >
                  <Box
                    role="button"
                    tabIndex={0}
                    onClick={() => navigate(`/app/workspace/${workspace.id}/products/${p.id}`)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        navigate(`/app/workspace/${workspace.id}/products/${p.id}`);
                      }
                    }}
                    sx={{
                      display: "flex",
                    ...
                    <Stack divider={<Box sx={{ borderTop: "1px solid", borderColor: "divider" }} />}>
                      {apps.map((a) => (
                        <Box
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/app/workspace/${workspace.id}/products/${p.id}/apps/${a.id}`);
                          }}
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === " ") {
                              e.preventDefault();
