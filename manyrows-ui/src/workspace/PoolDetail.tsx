...
type PoolUserRow = {
  ...
  email: string;
  enabled: boolean;
  apps: { id: string; name: string; productId: string }[];
};
...
export default function PoolDetail({ workspace, poolId, tab }: Props) {
  ...
  React.useEffect(() => {
    ...
    setUsersErr(null);
    axios
      .get<{ accounts: Array<{ id: string; email: string; enabled?: boolean; apps?: { id: string; name: string; productId: string }[] }>; total: number }>(
              ...
              {apps && apps.length > 0 && (
                <Stack spacing={0.5}>
                  {apps.map((a) => (
                    <RouterLink
                      key={a.id}
                      to={`/app/workspace/${workspace.id}/products/${a.productId}/apps/${a.id}`}
                            ...
                            {u.apps.length > 0 ? (
                              u.apps.map((a) => (
                                <Chip
                                  ...
                                  onClick={() =>
                                    navigate(
                                      `/app/workspace/${workspace.id}/products/${a.productId}/apps/${a.id}/members?email=${encodeURIComponent(u.email)}`,
                                    )
                                  }
