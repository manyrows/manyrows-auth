...
// Parse /app/workspace/:wsId[/products/:productId][/...] from the current pathname.
function parseRouteContext(pathname: string): { workspaceId?: string; productId?: string } {
  const m = pathname.match(/^\/app\/workspace\/([^/]+)(?:\/products\/([^/]+))?/);
  return { workspaceId: m?.[1], productId: m?.[2] };
}
...
export default function AppHeader(props: AppHeaderProps) {
  ...
  const { workspaceId, productId } = parseRouteContext(pathname);
  const currentWorkspace = workspaceId
    ? appData.workspaces.find((w) => w.id === workspaceId)
    : undefined;
  const currentProduct = currentWorkspace?.products?.find((p) => p.id === productId);
  ...
  const switchProduct = (id: string) => {
    if (workspaceId) navigate(`/app/workspace/${workspaceId}/products/${id}`);
    closeProj();
  };
      ...
      >
        ...
        >
          ...
          >
            ...
            {currentWorkspace && products.length > 0 && (
              <>
                <Crumb separator />
                <Button
                  onClick={openProj}
                  ...
                  sx={{
                    textTransform: "none",
                    color: currentProduct ? "text.primary" : "text.secondary",
                    fontWeight: currentProduct ? 600 : 500,
                    fontSize: 13,
                    minWidth: 0,
                    px: 0.75,
                    height: 28,
                  ...
                  {products.map((p) => (
                    ...
                    >
                      <ListItemIcon sx={{ minWidth: 26, color: "primary.main" }}>
                        {p.id === productId ? <Check size={13} strokeWidth={2.5} /> : null}
                      </ListItemIcon>
                      <Box sx={{ minWidth: 0, flex: 1 }}>
                        <Typography
                          sx={{
                            fontSize: 13,
                            fontWeight: p.id === productId ? 600 : 500,
                            overflow: "hidden",
                            textOverflow: "ellipsis",
                            whiteSpace: "nowrap",
                          }}
