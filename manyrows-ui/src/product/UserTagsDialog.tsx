...
export default function UserTagsDialog({
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    axios
      .get<TagsResponse>(`/admin/workspace/${workspaceId}/products/${productId}/apps/${appId}/tags`)
      .then((res) => {
  ...
  }, [open, workspaceId, productId, appId]);
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await axios.put<TagsResponse>(
        `/admin/workspace/${workspaceId}/products/${productId}/apps/${appId}/users/${userId}/tags`,
        { tags: value },
      );
