...
type App = {
  ...
  productId: string;
  type: AppType | string;
  // productName is the parent product's name, returned by the server
...
function appDisplayName(a: App, t: TFunc): string {
  ...
  switch (a.type) {
    ...
    case "staging":
      return `${a.productName} (Staging)`;
    case "dev":
      return `${a.productName} (Dev)`;
    default:
      return a.productName;
  }
}
...
export default function Apps({ project, workspace }: Props) {
  ...
  const appsBasePath = React.useMemo(() => {
    ...
    return `/admin/workspace/${workspaceId}/products/${productId}/apps`;
  ...
  const openAppDetail = React.useCallback((app: App) => {
    nav(`/app/workspace/${workspace.id}/products/${project.id}/apps/${app.id}`);
  }, [nav, workspace.id, project.id]);

  // ----- Create Dialog -----
  // Each app is an environment of the parent product (prod | staging |
  // dev). The display name comes from product + env type, so the form
  ...
  // The server enforces unique (product_id, type); reflect it in the
  ...
  const openCreate = () => {
    ...
    // for this product, so the form opens in a valid state.
  ...
  // free. Once all three exist for this product the only path forward
              ...
              <FormControl size="small" fullWidth>
                ...
                <FormHelperText>
                  {t("wizard.appEnv.help", {
                    defaultValue:
                      "Each product has up to one app per environment (prod, staging, dev). The display name comes from the product + this environment.",
                  })}
                </FormHelperText>
              </FormControl>
              <TextField
