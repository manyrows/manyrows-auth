...
export default function Branding({ project }: Props) {
  ...
  const mailtoHref = React.useMemo(() => {
    const subject = t("branding.mailSubject", {
      defaultValue: "Branding request — {{product}}",
      product: project.name,
    });
    const body = t("branding.mailBody", {
      defaultValue:
        "Hi ManyRows team,\n\nWe'd like to brand our sign-in experience for \"{{product}}\". Here's what we have in mind:\n\n- Brand colors:\n- Logo (attach or link):\n- Fonts:\n- Anything else:\n\nThanks!",
      product: project.name,
    });
    ...
    <Box>
      <PageHeader
        ...
        subtitle={t("branding.subtitle", {
          defaultValue:
            "Make the hosted sign-in, sign-up, and account screens look like your own product.",
        })}
        action={
          <Chip
            size="small"
