...
export default function AppDiff({ project }: Props) {
  ...
  const [leftAppId, setLeftAppId] = React.useState("");
  const [rightAppId, setRightAppId] = React.useState("");


  async function apiJson<T>(
    path: string,
    init?: { method?: string; data?: any; params?: any; headers?: Record<string, string> },
  ): Promise<T> {
    try {
        ...
        <>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ mb: 3 }}>
            <FormControl size="small" sx={{ minWidth: 200 }}>
              <InputLabel>{t("appDiff.selectLeft")}</InputLabel>
              <Select
                value={leftAppId}
                label={t("appDiff.selectLeft")}
                onChange={(e) => setLeftAppId(e.target.value)}
              >
                {apps.map((app) => (
                  <MenuItem key={app.id} value={app.id} disabled={app.id === rightAppId}>
            ...
            <FormControl size="small" sx={{ minWidth: 200 }}>
              ...
              >
                {apps.map((app) => (
                  <MenuItem key={app.id} value={app.id} disabled={app.id === leftAppId}>
                    {`${appDisplayName(app)} - ${appTypeLabel(app)}`}
                  </MenuItem>
                ))}
              </Select>
