...
interface Props {
  /** Page title - rendered in the sans (Geist) display style when a plain string. */
...
// PageHeader renders the editorial title block: a sans (Geist) title,
// optional subtitle/meta, and right-aligned actions. Centralizes the
// redesign typography so each page stops reinventing its own h1 styling.
  ...
  return (
    ...
    >
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <Box sx={{ display: "flex", alignItems: "center", mb: (subtitle || meta) ? 1.25 : 0 }}>
          {typeof title === "string" ? (
            <Typography
              sx={{
