...
import type { App, EnabledFilter, Permission, Project, ProjectMemberRole, Role, RoleFilter, WorkspaceMember } from "../core.ts";
...
function rolesUrl(project: Project) {
  return `/admin/workspace/${project.workspaceId}/projects/${project.id}/roles`;
}

function appsUrl(project: Project) {
  return `/admin/workspace/${project.workspaceId}/projects/${project.id}/apps`;
}

function permissionsUrl(project: Project) {
  return `/admin/workspace/${project.workspaceId}/projects/${project.id}/permissions`;
}

export function projectMemberRolesUrl(project: Project) {
  return `/admin/workspace/${project.workspaceId}/projects/${project.id}/memberRoles`;
}

// project members filtered by app (server-side)
export function projectMembersUrl(project: Project) {
  return `/admin/workspace/${project.workspaceId}/projects/${project.id}/members`;
}
...
export async function getProjectMembersPaged(
  project: Project,
  appId: string,
  page: number,
  pageSize: number,
  ...
  const r = await axios.get<ProjectMembersResponse>(projectMembersUrl(project), {
    params: {
      appId,
      page,
      pageSize,
...
async function getRoles(project: Project): Promise<Role[]> {
  const r = await axios.get(rolesUrl(project));
  return (r.data?.roles ?? r.data ?? []) as Role[];
}

async function getApps(project: Project): Promise<App[]> {
  const r = await axios.get(appsUrl(project));
  return (r.data?.apps ?? r.data ?? []) as App[];
}

async function getPermissions(project: Project): Promise<Permission[]> {
  const r = await axios.get(permissionsUrl(project));
  return (r.data?.permissions ?? r.data ?? []) as Permission[];
}

export async function getProjectMemberRoles(project: Project): Promise<ProjectMemberRole[]> {
  const r = await axios.get(projectMemberRolesUrl(project));
  return (r.data?.memberRoles ?? r.data ?? []) as ProjectMemberRole[];
}
...
export function useAppUsers(project: Project, appIdProp?: string) {
  ...
  const [memberRoles, setMemberRolesState] = React.useState<ProjectMemberRole[]>([]);
  ...
  const [projectMembers, setProjectMembers] = React.useState<WorkspaceMember[]>([]);
  const [projectMembersTotal, setProjectMembersTotal] = React.useState(0);
  ...
  const refreshCore = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const [r, fetchedApps, pmr, perms] = await Promise.all([
        getRoles(project),
        getApps(project),
  ...
  const refreshList = React.useCallback(async () => {
    ...
    setErr(null);
    try {
      const pmPaged = await getProjectMembersPaged(project, selectedAppId, memberPage, membersPerPage, memberSearchApplied, inactiveFilterDays, enabledFilter, roleFilter);
      if (reqId !== listReqId.current) return;
      setProjectMembers(pmPaged.members);
      setProjectMembersTotal(pmPaged.total);
    } catch (e) {
      if (reqId !== listReqId.current) return;
      setErr(extractApiError(e, t("projectMembers.failedToLoadList", { defaultValue: "Failed to load project members" })));
    } finally {
