...
import type {EnabledFilter, Permission, Project, Role, RoleFilter, UserField, WorkspaceMember} from "../core.ts";
import { appTypeLabel } from "../core.ts";
import { extractApiError } from "../lib/apiError.ts";
import { useAppUsers, getProjectMemberRoles, getProjectMembersPaged, projectMemberRolesUrl, projectMembersUrl } from "./useAppUsers.ts";
import {
  Alert,
  Autocomplete,
  Box,
...
function projectMemberRolesForAccountUrl(project: Project, accountId: string) {
...
async function resetUserTotp(workspaceId: string, projectId: string, appId: string, userId: string): Promise<void> {
  await axios.delete(`/admin/workspace/${workspaceId}/projects/${projectId}/apps/${appId}/users/${userId}/totp`);
}

async function unlockUserAccount(workspaceId: string, projectId: string, appId: string, userId: string): Promise<void> {
  await axios.post(`/admin/workspace/${workspaceId}/projects/${projectId}/apps/${appId}/users/${userId}/unlock`);
}

function appUserBase(workspaceId: string, projectId: string, appId: string, userId: string): string {
  return `/admin/workspace/${workspaceId}/projects/${projectId}/apps/${appId}/users/${userId}`;
}

async function adminSetUserPassword(workspaceId: string, projectId: string, appId: string, userId: string, password: string): Promise<void> {
  await axios.put(`${appUserBase(workspaceId, projectId, appId, userId)}/password`, { password });
}

async function adminSetUserEmailVerified(workspaceId: string, projectId: string, appId: string, userId: string, verified: boolean): Promise<void> {
  await axios.put(`${appUserBase(workspaceId, projectId, appId, userId)}/email-verified`, { verified });
}

async function adminCreateUserMagicLink(workspaceId: string, projectId: string, appId: string, userId: string): Promise<string> {
  const r = await axios.post<{ url: string }>(`${appUserBase(workspaceId, projectId, appId, userId)}/magic-link`, {});
  return r.data.url;
}

async function adminCheckUserPermission(workspaceId: string, projectId: string, appId: string, userId: string, permission: string): Promise<boolean> {
  const r = await axios.get<{ allowed: boolean }>(`${appUserBase(workspaceId, projectId, appId, userId)}/check-permission`, { params: { permission } });
...
function MemberRolesDialog(props: {
  ...
  // Direct (per-user) permission grants, managed alongside roles. Empty list =>
  // the project has no permissions defined and the section is hidden.
  permissions: Permission[];
  selectedPermIds: Set<string>;
  setSelectedPermIds: (next: Set<string>) => void;
              ...
              <>
                ...
                <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
                  <Typography variant="subtitle2">
                    {t("projectMembers.dialog.directPermissions", { defaultValue: "Direct permissions" })}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {t("projectMembers.dialog.permsSelected", { selected: selectedPermIds.size, total: permissions.length, defaultValue: "{{selected}} of {{total}} selected" })}
                  </Typography>
                  <Box sx={{ flex: 1 }} />
                  <Button size="small" onClick={() => setSelectedPermIds(new Set(permissions.map((p) => p.id)))}>
                    {t("projectMembers.dialog.selectAll")}
                  </Button>
                  <Button size="small" onClick={() => setSelectedPermIds(new Set())}>
                    {t("projectMembers.dialog.deselectAll")}
                  </Button>
                </Stack>

                <Typography variant="caption" color="text.secondary">
                  {t("projectMembers.dialog.directPermissionsHelp", { defaultValue: "Granted to this member directly, in addition to whatever their roles allow." })}
                </Typography>
                ...
                <Stack spacing={0.5}>
                  {permissions
                    ...
                    .map((p) => (
                      <FormControlLabel
                        key={p.id}
                        control={<Checkbox checked={selectedPermIds.has(p.id)} onChange={() => togglePerm(p.id)} />}
                        label={
                          <Stack spacing={0}>
...
export default function AppUsers({ project, appId: appIdProp }: Props) {
  ...
  const goApps = React.useCallback(() => {
    navigate(`/app/workspace/${project.workspaceId}/projects/${project.id}/apps`);
  }, [navigate, project.workspaceId, project.id]);

  const goRoles = React.useCallback(() => {
    navigate(`/app/workspace/${project.workspaceId}/projects/${project.id}/roles`);
  }, [navigate, project.workspaceId, project.id]);
  ...
  // Direct (per-user) permissions edited alongside roles in the member dialog.
  const [dialogPermIds, setDialogPermIds] = React.useState<Set<string>>(new Set());
  ...
  const loadIdentities = React.useCallback(async (userId: string, appId: string) => {
    ...
    try {
      const r = await axios.get<{ identities: UserIdentity[] }>(
        `/admin/workspace/${project.workspaceId}/projects/${project.id}/apps/${appId}/users/${userId}/identities`,
      );
  ...
  // to seed the member dialog's permission checkboxes in edit mode. Kept
  // separate from the project permission list (loaded once in useAppUsers) so a
  // failure here never blanks the list — the dialog still opens for editing.
  const getMemberDirectPermIds = React.useCallback(async (userId: string, appId: string): Promise<string[]> => {
    const res = await axios.get(`${memberPermsUrl}/${userId}`, { params: { appId } });
    return (res.data?.permissionIds ?? []) as string[];
  }, [memberPermsUrl]);
  ...
  const openEditDialog = (member: WorkspaceMember) => {
    ...
    // Seed direct (per-user) permissions from the server. A load failure is
    // surfaced but doesn't block editing roles.
    setDialogPermIds(new Set());
    if (permissions.length > 0) {
      void getMemberDirectPermIds(member.accountId, selectedAppId)
        .then((ids) => setDialogPermIds(new Set(ids)))
        .catch(() => enqueueSnackbar(t("projectMembers.permissionsLoadFailed", { defaultValue: "Could not load this member's permissions" }), { variant: "warning" }));
    }
    setDialogOpen(true);
  };
  ...
  const saveDialog = async () => {
    ...
    try {
      ...
      // Persist direct (per-user) permissions for the app. Edit mode always
      // writes so that unchecking clears a grant; add mode only writes when
      // something was selected, since a fresh membership starts with none.
      if (selectedAppId && permissions.length > 0 && (dialogMode === "edit" || dialogPermIds.size > 0)) {
        await axios.put(`${memberPermsUrl}/${accountId}`, {
          appId: selectedAppId,
          permissionIds: [...dialogPermIds],
        });
      }

      // refresh roles + list
      const pmr = await getProjectMemberRoles(project);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const [{ members: allMembers }, pmr, permsRes, fieldsRes] = await Promise.all([
        getProjectMembersPaged(project, selectedAppId, 0, 10000, ""),
        getProjectMemberRoles(project),
      ...
      for (let i = 0; i < allMembers.length; i += 10) {
        ...
        const batchResults = await Promise.all(batch.map(async (m) => {
          ...
          try {
            const dpRes = await axios.get(`/admin/workspace/${project.workspaceId}/projects/${project.id}/memberPermissions/${m.accountId}`, { params: { appId: selectedAppId } });
  ...
  return (
    <Box>
        ...
        <Stack direction="row" alignItems="flex-end" spacing={1.5} sx={{ flexWrap: "wrap", gap: 1 }}>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            {selectedApp?.userPoolName && (
              ...
              // can point at the same pool (SSO between projects);
      ...
      <MemberRolesDialog
        ...
        selectedPermIds={dialogPermIds}
        setSelectedPermIds={setDialogPermIds}
        onClose={closeDialog}
        onSave={saveDialog}
        saving={dialogSaving}
